import { queryCaseDynamicHistoryList } from '@/api/inter/interCase';
import { UserOutlined } from '@ant-design/icons';
import { Empty, theme, Tooltip, Typography } from 'antd';
import { FC, useEffect, useMemo, useState } from 'react';
import { IInterfaceCaseDynemic } from '../../types';
...
const CaseDynamic: FC<Props> = ({ case_id }) => {
  const [expanded, setExpanded] = useState(false);
  const [data, setData] = useState<IInterfaceCaseDynemic[]>([]);
  const { token } = theme.useToken();

  useEffect(() => {
    if (case_id) {
      queryCaseDynamicHistoryList(case_id).then((res) => {
        setData(res.data || []);
      });
    }
  }, [case_id]);
  ...
  const colors = {
    ...
    iconColor: isDark ? 'rgba(139, 148, 158, 0.8)' : '#8c8c8c',
  };
  ...
  return (
    <>
      ...
      <div
        className="case-dynamic-container"
        style={{
          padding: 24,
          minHeight: 300,
          borderRadius: 16,
            ...
            displayData.map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  >
                    <UserOutlined style={{ color: colors.iconColor }} />
                    <Text
                      strong
                      style={{
                        color: colors.accentPrimary,
